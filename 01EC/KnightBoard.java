...
class KnightBoard{
	...
	// }
	public static void main(String[] args) {
		KnightBoard a = new KnightBoard(20, 25);
		a.solveFast(dimX, dimY);
	...
	public static void solveFast(int rowsY, int colsX){
	...
	public static ArrayList<int[]> findSortedCoors(int[][] board, int row, int col){
		...
		for (int point = 0; point < possibleMoves.length; point++){            // for every potential move
			...
			while(search < out.size() + 1){
				...
				// System.out.println("out.get(search");
				// System.out.println(search);
				if (search >= out.size()){

					int[] entry = {possibleMoves[point][0], possibleMoves[point][1], possibleMoves[point][2]};
					out.add(entry);
					search += 999;
					continue;
